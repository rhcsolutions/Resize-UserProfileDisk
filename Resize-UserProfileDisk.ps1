...
#>
    ...
    Begin {
        ...
        if (![string]::IsNullOrEmpty($SingleVhdxFile)) {
            if ((Test-Path $SingleVhdxFile) -eq $false) {
                Write-Host "Cannot find file $SingleVhdxFile." -ForegroundColor Red
                Write-Log -severity Error -message "Cannot find file $SingleVhdxFile"
                return
            }
            $path = $SingleVhdxFile
            ...
            $measure_before = ((Get-Item $path).Length) / 1GB
    ...
    Process {
        ...
        if ($defrag) {
            ...
            foreach ($vhdx in $vhdxArray) {             
                ...
                Catch {
                    $errormessage = $_.exception.message
                    Write-Log -severity Error -message "Defrag: Try to mount VHDX-file $vhdx... Failed. Error: $errormessage" 
                    continue
                }

                try {
                    Write-Log -severity Information -message "Defrag: Checking for driveletter mounted $vhdx..." 
                    $DriveLetter = ($MountResult | Get-Disk | Get-Partition | Where-Object {$_.DriveLetter}).DriveLetter
                    if ([string]::IsNullOrEmpty($DriveLetter)) {
                        throw "No drive letter found for mounted VHDX"
                    }
                ...
                Catch {
                    ...
                    try {
                        Dismount-DiskImage $vhdx -ErrorAction SilentlyContinue | Out-Null
                    } catch {}
                    continue               
                }                    
                ...
                try {
                    ...
                    Catch {
                        $ErrorMessage = $_.exception.Message
                        Write-Log -severity Error -message "Defrag: Try to dismount VHDX-file $vhdx... Failed. Error: $ErrorMessage"
                    }
                }
                Catch {
                    $errormessage = $_.exception.message
        ...
        if ($ZeroFreeSpace) {
            ...
            foreach ($vhdx in $vhdxArray) {   
                ...
                Catch {
                    $errormessage = $_.exception.message
                    Write-Log -severity Error -message "SDelete: Try to mount VHDX-file $vhdx... Failed. Error: $errormessage" 
                    continue
                }
                ...
                Write-Log -severity Information -message "SDelete: Starting zero out of unused space from $vhdx... Mounted with driveletter $DriveLetter"
                & "$PSScriptRoot\sdelete.exe" -z -c "$($Driveletter):" -accepteula
                Do {
                ...
                Catch {
                    $ErrorMessage = $_.exception.Message
                    Write-Log -severity Error -message "SDelete: Try to dismount VHDX-file $vhdx... Failed. Error: $ErrorMessage"
                }
            } 
        }
